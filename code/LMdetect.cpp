...
DEFINE_string(video, "", "Use a video file instead of the camera. Use `examples/media/video.avi` for our default"
	" example video.");
DEFINE_string(image_dir, "F:/Jackie/Samsung_Proj/test", "Process a directory of images. Use `examples/media/` for our default example folder with 20"
...
DEFINE_string(camera_parameter_folder, "F:/Jackie/SDKs/openpose/models/cameraParameters/flir/", "String with the folder where the camera parameters are located.");
...
// OpenPose
DEFINE_string(model_folder, "F:/Jackie/SDKs/openpose/models/", "Folder path (absolute or relative) where the models (pose, face, ...) are located.");
...
void LMdetector::openPoseTutorialWrapper1(std::vector<std::vector<Eigen::Vector2f>>& result)
{
	try
	{
		...
		while (!userWantsToExit)
		{
			...
			{
				userWantsToExit = input.display(datumProcessed);;
				std::vector<Eigen::Vector2f> faceLandmarks;
				//input.printKeypoints(datumProcessed);
				std::cout << datumProcessed->at(0).faceKeypoints.getVolume() << std::endl;
				for (int i = 0; i < datumProcessed->at(0).faceKeypoints.getVolume(); i = i + 3)
				{
					x = datumProcessed->at(0).faceKeypoints[i];
					y = datumProcessed->at(0).faceKeypoints[i + 1];
					faceLandmarks.push_back(Eigen::Vector2f(x, y));
				}
				//std::cout << faceLandmarks.size() << std::endl;
				result.push_back(faceLandmarks);
